...
import {cppCode, cSharpCode, javaCode, javascriptCode, pythonCode} from "./BubbleSortCode.jsx";
import {BubbleSortFunction} from "./BubbleSortFunction.jsx";

function BubbleSort() {
    const [inputArray, setInputArray] = useState('');
    const [arrayLength, setArrayLength] = useState(null);
    const [boxesArray, setBoxesArray] = useState([]);
    const [currentCorrect, setCorrect] = useState(null);
    ...
    const handleSubmit = () => {
        ...
        if (array.some(isNaN)) {
            ...
            BubbleSortFunction(array, (array, i) => {
                const boxes = array.map((value, id) => ({id, value}));
                setBoxesArray(boxes);
                setCorrect(array.length - i);
            });
        }
    };
